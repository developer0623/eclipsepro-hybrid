 ...
 * @ngInject
 */
 AmsMdTabsController.$inject = [
   '$scope', '$element', '$window', '$mdConstant', '$mdTabInkRipple', '$mdUtil',
   '$animateCss', '$attrs', '$compile', '$mdTheming', '$mdInteraction', '$timeout',
   'AmsMdTabsPaginationService'
 ]
export function AmsMdTabsController ($scope, $element, $window, $mdConstant, $mdTabInkRipple, $mdUtil,
                           $animateCss, $attrs, $compile, $mdTheming, $mdInteraction, $timeout,
                           AmsMdTabsPaginationService) {
  ...
  function defineOneWayBinding (key, handler) {
    let attr = $attrs.$normalize('md-' + key);
   ...
   */
  function defineBooleanAttribute (key, handler = undefined) {
    let attr = $attrs.$normalize('md-' + key);
  ...
  function handleStretchTabs () {
    ...
    angular.element((elements as any).wrapper).toggleClass('md-stretch-tabs', shouldStretchTabs());
    updateInkBarStyles();
  }
  ...
  function handleOffsetChange (left) {
    let newValue = ((ctrl.shouldCenterTabs || isRtl() ? '' : '-') + left + 'px');

    // Fix double-negative which can happen with RTL support
    newValue = newValue.replace('--', '');

    angular.element((getElements() as any).paging).css($mdConstant.CSS.TRANSFORM,
  ...
  function handleFocusIndexChange (newIndex, oldIndex) {
    ...
    if (!(getElements() as any).tabs[ newIndex ]) return;
    adjustOffset();
    redirectFocus();
  }
  ...
  function getTabElementIndex(tabEl){
    let tabs = $element[0].getElementsByTagName('ams-md-tab');
  ...
  function handleResizeWhenVisible () {
    ...
    if ((handleResizeWhenVisible as any).watcher) return;
    // otherwise, we will abuse the $watch function to check for visible
    (handleResizeWhenVisible as any).watcher = $scope.$watch(function () {
      ...
      $mdUtil.nextTick(function () {
        ...
        if ($element.prop('offsetParent')) {
          (handleResizeWhenVisible as any).watcher();
          (handleResizeWhenVisible as any).watcher = null;
   ...
   */
  function select (index, canSkipClick = undefined) {
  ...
  function nextPage () {
    ...
    let newOffset = AmsMdTabsPaginationService.increasePageOffset(getElements(), ctrl.offsetLeft);
  ...
  function previousPage () {
    ...
    let newOffset = AmsMdTabsPaginationService.decreasePageOffset(getElements(), ctrl.offsetLeft);
  ...
  function handleInkBar (hide) {
    angular.element((getElements() as any).inkBar).toggleClass('ng-hide', hide);
  }
  ...
  function removeTab (tabData) {
    if (destroyed) return;
    let selectedIndex = ctrl.selectedIndex,
  ...
  function insertTab (tabData, index) {
    ...
    let tab = angular.extend(proto, tabData);
  ...
  function getElements () {
    ...
    elements.tabs    = elements.paging.querySelectorAll('ams-md-tab-item');
  ...
  function getFocusedTabId() {
    let focusedTab = ctrl.tabs[ctrl.focusIndex];
  ...
  function shouldPaginate () {
    ...
    let canvasWidth = $element.prop('offsetWidth');
  ...
  function getNearestSafeIndex (newIndex) {
    ...
    let maxOffset = Math.max(ctrl.tabs.length - newIndex, newIndex),
        i, tab;
   ...
   * @param {any} value
   */
  function defineProperty (key, handler, value = undefined) {
  ...
  function updateTabOrder () {
    let selectedItem   = ctrl.tabs[ ctrl.selectedIndex ],
  ...
  function redirectFocus () {
    ...
    let tabToFocus = getElements().tabs[ctrl.focusIndex];
    if (tabToFocus) {
      tabToFocus.focus();
    }
  }
  ...
  function adjustOffset (index = 0) {
    ...
    let tab         = elements.tabs[ index ],
        ...
        // extraOffset = 32;
        extraOffset = ctrl.shouldPaginate ? elements.canvas.offsetWidth / 2 - tab.offsetWidth/2: 32;
    ...
    if (isRtl()) {
      let tabWidthsBefore = calcTabsWidth(Array.prototype.slice.call(elements.tabs, 0, index));
      let tabWidthsIncluding = calcTabsWidth(Array.prototype.slice.call(elements.tabs, 0, index + 1));
  ...
  function updateHeightFromContent () {
    ...
    let tabContent    = elements.contents[ ctrl.selectedIndex ],
    ...
    let fromHeight = { height: currentHeight + 'px' },
   ...
   */
  function updateInkBarStyles (previousTotalWidth = 0, previousWidthOfTabItems = 0) {
    if (ctrl.noInkBar) {
      return;
    }
    let elements = getElements();
    ...
    if (ctrl.shouldCenterTabs) {
      ...
      // deviations.
      let totalWidthOfTabItems = calcTabsWidth(elements.tabs);
  ...
  function updateInkBarClassName () {
    let elements = getElements();
    let newIndex = ctrl.selectedIndex,
  ...
  function attachRipple (scope, element) {
    let elements = getElements();
    let options = { colorElement: angular.element(elements.inkBar) };
  ...
  function setAriaControls (tab) {
    if (tab.hasContent) {
      let nodes = $element[0].querySelectorAll('[md-tab-id="' + tab.id + '"]');
