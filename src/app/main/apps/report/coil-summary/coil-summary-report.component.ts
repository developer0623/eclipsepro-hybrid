...
import ReportTemplate from './coil-summary-report.html';

const CoilSummaryReport_ = {
  selector: 'coilSummaryReport',
  bindings: {},
  template: ReportTemplate,
  /** @ngInject */
  controller: ['$mdPanel', '$mdDialog', 'clientDataStore', 'apiResolver', 'appService', '$location', '$httpParamSerializer',  class CoilSummaryReportComponent {
    _mdPanel;
    ...
    constructor(
      ...
      let lsDuration = localStorage.getItem('report.coilSummary.Duration');
      ...
      const sub_ = reportFilterSubject$
        ...
        .map(filters => {
          ...
          return Rx.Observable.fromPromise(
            apiResolver.resolve<CoilSummary[]>('report.coilSummary@get', query)
          );
        })
        .switch()
        .subscribe(report => {
...
export default CoilSummaryReport_;
